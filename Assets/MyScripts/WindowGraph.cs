...
public class WindowGraph : MonoBehaviour {
    ...
    public void Show(List<System.Numerics.Vector2> valueList) {
        ...
        for (int i = 0; i < valueList.Count; i++) {
            ...
            if (value > yMaximum)
                yMaximum = value;
            else if (value < yMinimum)
                yMinimum = value;

            value = valueList[i].X;
            if (value > xMaximum)
                xMaximum = value;
            else if (value < xMinimum)
                xMinimum = value;
        }
        ...
        for (int i = 0; i <= separatorCount; i++) {
            ...
            if (i == separatorCount) {
                if (yName != string.Empty)
                    labelY.GetComponent<Text>().text = yName;
                if (xName != string.Empty)
                    labelX.GetComponent<Text>().text = xName;
            }
